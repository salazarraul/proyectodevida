...
function Producto (codigo, nombre, precio, cantidad,  imagen) {
this.codigo = codigo,
this.nombre = nombre,
this.precio = precio,
...
function addProductoArray(){
    ...
    var producto = new Producto(codigo, nombre, precio, cantidad, imagen);
...
function LlenarTabla(){
    ...
    for (let index = 0; index < productos.length; index++) {
        ...
        scriptTabla += "<td>" + productos[index].cantidad + "</td>" ;
...
function LlenarTablaP(){
    ...
    scriptTabla += '<button type="button" class="btn btn-success" onclick="addCarrito('+productos[index].codigo+')">Agregar al Informe</button>';
    scriptTabla += '</td>';
    scriptTabla += '<td>' + productos[index].cantidad + '</td>';
    scriptTabla += '<td><img src="'+ productos[index].imagen + '"/></td>';
    scriptTabla += '</tr>'
...
var ruta = "";
//convierte la imagen cargada en el input */
